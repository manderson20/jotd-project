/**
 * Cloudflare Worker: Joke of the Day (GitHub-backed)
 *
 * Public:
 *   GET  /            -> info
 *   GET  /health      -> info
 *   GET  /admin       -> admin UI (public page)
 *
 * Protected (X-API-Key == env.JOTD_API_KEY):
 *   GET  /v1/joke/today
 *   GET  /v1/joke/random
 *   GET  /v1/joke?id=123
 *   GET  /v1/admin/jokes
 *   POST /v1/admin/jokes  { text, rating, category, active }
 *   GET  /v1/admin/debug  -> shows which env vars are set (safe output; no secrets)
 *
 * Required Secrets:
 *   JOTD_API_KEY
 *   GITHUB_TOKEN  (fine-grained PAT: Contents Read/Write for this repo)
 *
 * Required Vars:
...
var RAW_JOKES_URL = "https://raw.githubusercontent.com/manderson20/jotd-project/main/jokes.json";
    ...
    try {
      ...
      if (startsWith(url.pathname, "/v1/")) {
        ...
        if (required && provided !== required) {
          // always JSON so the UI can show the message
          return json({ error: "Unauthorized" }, 401);
        }
      }

      // ---------- Debug (protected) ----------
      if (url.pathname === "/v1/admin/debug") {
        return json({
          ...
          JOKES_PATH: env.JOKES_PATH || "jokes.json",
          note: "If has_GITHUB_TOKEN is false, admin list/add cannot work."
        }, 200);
      }
      ...
      // NEVER crash the worker with an unhandled exception
      return json({ error: "Internal error", detail: toMessage(err) }, 500);
    }
  }
};
...
async function fetchJokesFromRaw(ctx) {
  ...
  var cacheKey = new Request(RAW_JOKES_URL, { method: "GET" });

  var res = await cache.match(cacheKey);
  if (!res) {
    res = await fetch(RAW_JOKES_URL, { headers: { "User-Agent": "jotd-worker" } });
...
// ------------------ Admin API (GitHub read/write) ------------------

async function handleAdminList(env) {
  try {
    var read = await githubReadJokesFile(env);
    return json({ count: read.jokes.length, jokes: read.jokes }, 200);
  } catch (err) {
    return json({ error: "Admin list failed", detail: toMessage(err) }, 500);
  }
}
...
async function handleAdminAdd(request, env) {
  try {
    ...
    return json({ error: "Admin add failed", detail: toMessage(err) }, 500);
  }
}
...
async function githubWriteJokesFile(env, sha, newContent, message) {
  ...
  var body = { message: message, content: btoa(newContent), sha: sha, branch: branch };
...
function renderAdminHtml() {
  return (
    ...
    '<select id="rating"><option>G</option><option>PG</option><option>PG-13</option><option>R</option></select></div>' +
    '<div><label>Category</label><br/>' +
    '<input id="category" placeholder="tech, dad, school, pun..."/></div>' +
    '<div><label>Active</label><br/>' +
    '<select id="active"><option value="true" selected>true</option><option value="false">false</option></select></div>' +
    '</div>' +
    ...
    '<p id="count"></p>' +
    '<table id="table" style="display:none"><thead><tr>' +
    '<th>ID</th><th>Rating</th><th>Category</th><th>Active</th><th>Text</th>' +
    ...
    'if(!res.ok){setStatus("Failed",false);setError(JSON.stringify({status:res.status,body:data},null,2));return;}' +
    ...
    'body:JSON.stringify({text:text,rating:rating,category:category,active:active})});' +
    'const data=await res.json().catch(()=>({error:"Non-JSON response from server"}));' +
    'if(!res.ok){setStatus("Failed",false);setError(JSON.stringify({status:res.status,body:data},null,2));return;}' +
...
function formatDateInTZ(date, timeZone) {
  ...
  var y = part(parts, "year");
  var m = part(parts, "month");
  var d = part(parts, "day");
  return y + "-" + m + "-" + d;
}
